...
var stuffToDo;
			...
			function returnYears(birthday) //birthday is of type YYYYMMDD like 19870603
			{

				//assume life expectancy to be 60 years
				...
				return Math.ceil(ageExpectancy - Math.ceil(ageDifMs/(1000*60*60*24*355)));
...
function plotLifeChart()
{
	...
	// richasdy
	// number of row at plot
	// roundup(ageExpectancy/20)
	plot_row_num = Math.ceil(ageExpectancy/20);
	...
	for(var row=0; row<plot_row_num; row++)
	{
...
function fillstuff(elem,stuff)
{
  		...
  		function calculateEverything()
  		{
  			...
  			{	
  				...
  				yearsLeft=returnYears(localStorage.bdayInput);


  				if(localStorage.nameInput) prependString=localStorage.nameInput+", umurmu hanya tersisa ";
  				else prependString="Umurmu hanya tersisa "


  					$('#weeksLeft').html(prependString+yearsLeft+" tahun ("+weeksLeft+" minggu) lagi");

  				$('#title').html('Bagaimana kamu akan menggunakan waktumu'+(localStorage.nameInput ? " "+localStorage.nameInput : "")+"?");
  			...
  			} else
  			{
  				...
  				$('#weeksLeft').html("Tolong beri tahu kami siapa nama Anda dan kapan Anda lahir");
